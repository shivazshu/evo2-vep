...
def run_brca1_analysis():
    ...
    brca1_df = pd.read_excel('/evo2/notebooks/brca1/41586_2018_461_MOESM3_ESM.xlsx',
    header=2,
    )
    brca1_df = brca1_df[[
...
def brca1_example():
    ...
    print("Running BRCA1 variant analysis using Evo2..")
...
def analyze_variant(relative_pos_in_window, reference, alternative, window_seq, model):
    ...
    threshold = -0.0005284548
    lof_std = 0.0012889444
    func_std = 0.0012889444
...
def get_genome_seq(position, genome: str, chromosome: str, window_size: 8192):
    ...
    if (response.status_code != 200):
        raise Exception(f"Failed to fetch sequence from UCSC API: {response.status_code}")

    genome_data = response.json()

    if "dna" not in genome_data:
        error = genome_data.get("error", "Unknown error")
        raise Exception(f"UCSC API raised an error: {error}")
...
class Evo2Model:
    ...
    def analyze_single_variant(self, variant_pos: int, alternative: str, genome: str, chromosome: str):
        ...
        window_seq, seq_start = get_genome_seq(
        ...
        if relative_pos < 0 or relative_pos >= len(window_seq):
            raise ValueError(f"Relative position is outside the fetched window: (start={seq_start}, end={seq_start + window_seq})")
...
def main():
    ...
    result = evo2_model.analyze_single_variant.remote(variant_pos=43119628, alternative="G", genome="hg38", chromosome="chr17")
